<?php
// ملف: gallery-schema.php

// 1. إضافة صندوق الميتا لمعرض الصور مع خيارات التفعيل
function veel_add_gallery_meta_box() {
    add_meta_box(
        'veel_gallery_meta_box',
        __('معرض الصور وسكيما المعرض', 'veelinvestments'),
        'veel_gallery_meta_box_callback',
        array('post', 'projects', 'units'), // أنواع المنشورات التي تريد تطبيقها عليها
        'normal',
        'high'
    );
}
...
function veel_gallery_meta_box_callback($post) {
    ...
    <p>
        <label for="gallery_enabled">
            ...
            <?php _e('تفعيل عرض المعرض', 'veelinvestments'); ?>
        </label>
    </p>
    <p>
        <label for="gallery_schema_enabled">
            ...
            <?php _e('تفعيل سكيما المعرض', 'veelinvestments'); ?>
        </label>
    </p>
    <div id="gallery-wrapper">
        ...
        <button id="add-gallery-images" class="button"><?php _e('إضافة صور', 'veelinvestments'); ?></button>
    </div>
    ...
    <script>
        jQuery(document).ready(function($) {
            ...
            $('#add-gallery-images').on('click', function(e) {
                ...
                frame = wp.media({
                    title: '<?php _e('اختر الصور', 'veelinvestments'); ?>',
                    button: {
                        text: '<?php _e('إضافة إلى المعرض', 'veelinvestments'); ?>'
                    },
                    multiple: true
                });
...
// 3. حفظ بيانات المعرض وخيارات التفعيل
function veel_save_gallery_meta_box($post_id) {
    ...
    // حفظ حالة تفعيل المعرض والسكيما
...
// 4. عرض المعرض يدوياً في المكان المطلوب
function veel_display_gallery_or_featured_image($post_id, $size = 'full') {
    ...
    if ($gallery_enabled === '1' && !empty($gallery_image_ids) && is_array($gallery_image_ids)) {
        ...
        if ($total_images > 3) {
            ...
            echo '<div class="newaqar-gallery-grid newaqar-gallery-grid-more">';
        } else {
            echo '<div class="newaqar-gallery-grid newaqar-gallery-grid-thrd">';
        }
        ...
        if (has_post_thumbnail($post_id)) {
            ...
            if ($featured_image_id) {
                ...
                if ($featured_image_url && $image_data) {
                    ...
                    echo '<div class="newaqar-gallery-single">';
...
function veel_gallery_shortcode($atts) {
    ob_start();
    $post_id = get_the_ID();
    veel_display_gallery_or_featured_image($post_id, 'full');
    return ob_get_clean();
}
add_shortcode('veel_gallery', 'veel_gallery_shortcode');
...
function veel_output_image_gallery_schema() {
    if (is_singular(array('post', 'projects', 'units'))) {
        ...
        if ($gallery_schema_enabled === '1' && !empty($gallery_image_ids) && is_array($gallery_image_ids)) {
            ...
            echo wp_json_encode($ld_json, JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
            echo '</script>';
        }
    }
}
