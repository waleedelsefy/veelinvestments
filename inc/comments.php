 ...
 * Comment reply
 */
function veelinvestments_reply() {
  if (is_singular() && comments_open() && get_option('thread_comments')) {
    wp_enqueue_script('comment-reply');
  }
}
add_action('wp_enqueue_scripts', 'veelinvestments_reply');
/**
 * Comments
 */
if (!function_exists('veelinvestments_comment')) :
  /**
   * Template for comments and pingbacks.
   *
   * Used as a callback by wp_list_comments() for displaying the comments.
   */
  function veelinvestments_comment($comment, $args, $depth) {
    // $GLOBALS['comment'] = $comment;
    if ('pingback' == $comment->comment_type || 'trackback' == $comment->comment_type) : ?>
      <li id="comment-<?php comment_ID(); ?>" <?php comment_class('media alert alert-info'); ?>>
      <div class="comment-body">
        <?php _e('Pingback:', 'veelinvestments'); ?><?php comment_author_link(); ?><?php edit_comment_link(__('Edit', 'veelinvestments'), '<span class="edit-link">', '</span>'); ?>
      </div>
    <?php else : ?>
      <li id="comment-<?php comment_ID(); ?>" <?php comment_class(empty($args['has_children']) ? '' : 'parent'); ?>>
        <article id="div-comment-<?php comment_ID(); ?>" class="comment-body mb-4 d-flex">
          ...
          <div class="comment-content">
            <div class="card">
              <div class="card-body">
                <div class="mt-0"><?php printf(__('%s <span class="says d-none">says:</span>', 'veelinvestments'), sprintf('<h3 class="h5">%s</h3>', get_comment_author_link())); ?>
                </div>
                <p class="small comment-meta text-secondary">
                  <time datetime="<?php comment_time('c'); ?>">
                    <?php printf(_x('%1$s at %2$s', '1: date, 2: time', 'veelinvestments'), get_comment_date(), get_comment_time()); ?>
                  </time>
                  <?php edit_comment_link(__('Edit', 'veelinvestments'), '<span class="edit-link">', '</span>'); ?>
                </p>
                <?php if ('0' == $comment->comment_approved) : ?>
                  <p class="comment-awaiting-moderation alert alert-info"><?php _e('Your comment is awaiting moderation.', 'veelinvestments'); ?></p>
                <?php endif; ?>
                <?php comment_text(); ?>
...
function wp44138_change_comment_form_cookies_consent($fields) {
  ...
  $fields['cookies'] = '<p class="comment-form-cookies-consent custom-control form-check mb-3">' .
                       ...
                       '<label for="wp-comment-cookies-consent" class="form-check-label">' . __('Save my name, email, and website in this browser for the next time I comment.', 'veelinvestments') . '</label>' .
                       '</p>';
  return $fields;
}
 ...
 * Comment Button
 */
if (!function_exists('veelinvestments_comment_button')) :
  function veelinvestments_comment_button($args) {
    $args['class_submit'] = 'btn btn-outline-primary'; // since WP 4.1
    return $args;
  }
  add_filter('comment_form_defaults', 'veelinvestments_comment_button');
endif;
